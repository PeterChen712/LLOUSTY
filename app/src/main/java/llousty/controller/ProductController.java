...
public class ProductController extends DbConfig{
    ...
    //CREATE
    public static boolean addProduct(String name, String category, int stock, double price, File selectedFile, int sellerId){
        query = "INSERT INTO product (name, category, stock, price, productPhoto, sellerId) VALUES (?, ?, ?, ?, ?, ?)";
        try {
            getConnection();
    ...
    public static boolean updateProduct(int id, String name, String description, String category, int stock, double rate, double price, File selectedFile, int totalRate, int sellerId){
        query = "UPDATE product SET name=?, description=?, category=?, stock=?, rate=?, price=?, productPhoto=? WHERE id=?";
        try {
            getConnection();
            preparedStatement = connection.prepareStatement(query);
            preparedStatement.setString(1, name);
